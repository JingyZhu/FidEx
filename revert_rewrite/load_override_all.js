...
const errorFixAll = require('./error-fixall');
...
(async function(){
    ...
    try {
        ...
        let {result, log, logFixAll, workingOverrides: newWorkingOverrides} = await loadAndFix(url, page, client, 'load', options, loadFunc);
        if (!result['success'])
