...
def test_no_issue_gt():
    dirs = [
        ...
        # 'usda-fsa.usajobs.gov_18db8a141d', # * Rerun the record replay, solved (unclear why originally there is a problem)
        # 'ssaiseattle.usajobs.gov_eb0e05b0b6', # * Similar to usda, loading on original URL auto-redirect (no navigation) to with query
        # 'www.kyeb.uscourts.gov_c42759e5f6', # * Rerun the record replay, cannot reproduce
        'www.morganlehmangallery.com_1e48d26ec2_mr'
    ]
    for dirr in dirs:
        print(dirr)
...
def test_no_issue_multi_replay_gt():
    ...
    for i, url in enumerate(urls):
        ...
        ts, url = replay_multi_times(url, archive_name, wr_archive='test', pw_archive='test', remote_host=REMOTE, times=check_times)
        dirr = archive_name
        call(f'cp -r {home_dir}/fidelity-files/writes/test/{dirr} {base}/{dirr}', shell=True)
        full_dir = os.path.join(base, dirr)
        dir_issue[dirr] = []
