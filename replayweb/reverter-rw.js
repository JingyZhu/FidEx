...
const fs = require('fs');
const { browserFetcher } = require('../utils/load');
...
function addHostname(url, hostname) {
    ...
    if (pathParts.length < 4) 
        return url;
    let origURL = pathParts.slice(4).join('/');
    ...
    urlObj.pathname = `${pathParts.slice(0, 4).join('/')}/${origURL}`;
...
class Reverter {
    ...
    async revertFile2Original(url) {
        ...
        const res = await browserFetcher.fetch(origURL);
    ...
    async revert404response(url, hostname) {
        ...
        if (newURL === url) 
            return null;
        // Fetch origURL and return the content
        const res = await browserFetcher.fetch(newURL);
