...
async function interaction(page, cdp) {
    ...
    let numEvents = await page.evaluate(async () => {
        eli.init();
        eli.shuffle();
        return eli.final_listeners.length;
    })
    return numEvents
}
...
(async function(){
    ...
    try {
        ...
        let numEvents = await interaction(recordPage, client);
        ...
        console.log("Number of event listeners:", numEvents);
