...
(function () {
    ...
    function binaryToString(data) {
      ...
      } else string = data ? data.toString() : "";
    ...
    function postToGetUrl(request) {
      ...
      switch (requestMime) {
        ...
        case "multipart/form-data":
          query = mfdToQueryString(
            decodeIfNeeded(postData),
    ...
    function jsonToQueryParams(json, ignoreInvalid = true) {
      ...
      const q = new URLSearchParams(),
        ...
        getKey = (key) =>
          q.has(key)
            ? (dupes[key] || (dupes[key] = 1), key + "." + ++dupes[key] + "_")
            : key;
      try {
        JSON.stringify(
          json,
          (k, v) => (
            ["object", "function"].includes(typeof v) || q.set(getKey(k), v), v
          )
        );
      } catch (e) {
    ...
    function mfdToQueryParams(mfd, contentType) {
      ...
      mfd instanceof Uint8Array && (mfd = new TextDecoder().decode(mfd));
      try {
        const boundary = contentType.split("boundary=")[1],
          parts = mfd.split(new RegExp("-*" + boundary + "-*", "mi"));
    ...
    (Storage.prototype.getItem = function getItem(name) {
      ...
      (Wombat.prototype.skipWrapScriptBasedOnType = function (scriptType) {
        return (
          ...
          (!!(scriptType.indexOf("json") >= 0) ||
            scriptType.indexOf("text/template") >= 0)
        );
      }),
      ...
      (Wombat.prototype.resolveRelUrl = function (url, doc) {
        ...
        return (
          (parser.href =
            lastslash >= 0 && lastslash !== href.length - 1
              ? href.substring(0, lastslash + 1) + url
              : href + url),
          parser.href
        );
      }),
      ...
      (Wombat.prototype.defaultProxyGet = function (
        ...
        return type === "object" && retVal && retVal._WB_wombat_obj_proxy
          ? (retVal instanceof Window && this.initNewWindowWombat(retVal),
      ...
      (Wombat.prototype.rewriteScript = function (elem) {
        ...
        return (
          ...
          ((elem.textContent = this.wrapScriptTextJsProxy(text)), true)
        );
      }),
              ...
              if (
                ...
                if (
                  ...
                  elem.src =
                    ...
                    "id_/srcdoc:" +
                    btoa(encodeURIComponent(srcdoc));
                else {
      ...
      (Wombat.prototype.rewriteDocWriteWriteln = function (
        ...
        string =
          argLen === 1 ? argsObj[0] : Array.prototype.join.call(argsObj, "");
      ...
      (Wombat.prototype.overrideHistoryFunc = function (funcName) {
        ...
        var wombat = this,
          rewrittenFunc = function histNewFunc(stateObj, title, url) {
            var rewritten_url,
              resolvedURL,
              historyWin = this.___wb_ownWindow || wombat.$wbwindow,
        ...
        if (this.$wbwindow.Blob && !this.wb_info.isSW) {
          ...
          (this.$wbwindow.Blob = (function (Blob_) {
            return function Blob(array, options) {
              return (
                ((options && options.type === "application/xhtml+xml") ||
      ...
      (Wombat.prototype.initWorkerOverrides = function () {
        ...
        if (
          ...
          !this.$wbwindow.Worker._wb_worker_overriden
        ) {
          ...
          windowProxy = new $wbwindow.Proxy(
            {},
            {
              ...
              defineProperty: function (target, prop, desc) {
                var ndesc = desc || {};
                return (
                  ndesc.value || ndesc.get || (ndesc.value = $wbwindow[prop]),
      ...
      (Wombat.prototype.wombatInit = function () {
        this._internalInit(),
          ...
          this.initFixedRatio(this.wb_info.pixel_ratio || window.devicePixelRatio || 1),
