...
def start_http_warc_server(warc_path):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(('', 0))
        port = s.getsockname()[1]
    serve_process = Popen(['http-server', warc_path, '-p', str(port), '--cors', '*'], 
             cwd=_FILEDIR, stdout=PIPE, stderr=PIPE)
    return serve_process, port
...
def record_replay(url, archive_name,
    ...
    #     a_pw_archive = wb_manager.collection(pw_archive)npm
